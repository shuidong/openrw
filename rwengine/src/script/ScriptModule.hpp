...
struct binder<bool, Targs...> {
  ...
  static void call(Tfunc func, const ScriptArguments& args)
  {
    args.getThread()->conditionResult =
        call_unpacked<sizeof...(Targs), 0, bool, Targs...>::call(func, args);
  }
};
 ...
 * interface.
 *
 * For example a collection of functions that control the time of day, or create objects
 * would
 * be the collected within one ScriptModule with a sensible name like "Environment" or
 * "Objects"
 */
class ScriptModule
{
