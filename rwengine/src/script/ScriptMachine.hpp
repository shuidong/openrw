...
struct IllegalInstruction : SCMException {
  ...
  {
    std::stringstream ss;
    ss << "Illegal Instruction " << std::setfill('0') << std::setw(4) << std::hex
       << opcode << " encountered at offset " << std::setfill('0') << std::setw(4)
       << std::hex << offset << " on thread " << thread;
    return ss.str();
  }
};
...
struct UnknownType : SCMException {
  ...
  std::string what() const
  {
    ...
    ss << "Unkown data type " << std::setfill('0') << std::hex
       ...
       << std::setfill('0') << std::hex << offset << " on thread " << thread;
    return ss.str();
  }
};
 ...
 * by consuming the correct number of arguments, allowing the next instruction to be
 * found,
