...
inline void getObjectPosition(GameObject* object, ScriptFloat& x, ScriptFloat& y,
                              ScriptFloat& z)
{
...
inline bool isInModel(const ScriptArguments& args, CharacterObject* character, int model)
{
  ...
  if (data) {
    ...
    if (vehicle) {
      return vehicle->model ? vehicle->model->name == data->modelName : false;
    }
  }
  return false;
}
...
bool objectInArea(const ScriptArguments& args, const Tobj& object, const Tvec& pointA,
                  const Tvec& pointB, bool marker, bool preconditions = true)
{
...
inline bool objectInRadiusNear(const ScriptArguments& args, GameObject* object,
                               GameObject* obj_near, const Tvec& radius, bool marker,
                               bool preconditions = true)
{
...
inline BlipData createObjectBlipSprite(const ScriptArguments& args, GameObject* object,
                                       int sprite)
{
  BlipData data;
