    ...
    , text(this)
{
  ...
  for (int s = 0; s < cylsegments; ++s) {
    ...
    cylverts.push_back({glm::vec3(p0, 2.f), glm::vec3(), glm::vec2(0.45f, 0.6f),
                        glm::u8vec4(255, 255, 255, 50)});
    cylverts.push_back({glm::vec3(p0, -1.f), glm::vec3(), glm::vec2(0.45f, 0.4f),
                        glm::u8vec4(255, 255, 255, 150)});
    cylverts.push_back({glm::vec3(p1, 2.f), glm::vec3(), glm::vec2(0.55f, 0.6f),
                        glm::u8vec4(255, 255, 255, 50)});
...
void GameRenderer::renderWorld(GameWorld* world, const ViewCamera& camera, float alpha)
{
  ...
  std::sort(renderList.begin(), renderList.end(),
            [](const Renderer::RenderInstruction& a,
               const Renderer::RenderInstruction& b) { return a.sortKey < b.sortKey; });
  RW_PROFILE_END();
...
bool GameRenderer::renderFrame(Model* m, ModelFrame* f, const glm::mat4& matrix,
                               GameObject* object, float opacity, bool queueTransparent)
{
