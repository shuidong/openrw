...
bool SaveGame::loadGame(GameState& state, const std::string& file)
{
  ...
  for (int c = 0; c < 18; ++c) {
    ...
    std::cout << " " << car.modelId << " " << uint16_t(car.colorFG) << "/"
              << uint16_t(car.colorBG) << " " << car.immunities << std::endl;
  }
#endif
  ...
  for (size_t v = 0; v < vehicleCount; ++v) {
    ...
    std::cout << " v " << veh.modelId << " " << veh.state.position.x << " "
              << veh.state.position.y << " " << veh.state.position.z << std::endl;
#endif
  }
  ...
  for (size_t o = 0; o < objectCount; ++o) {
    ...
    glm::vec3 right =
        glm::normalize(glm::vec3(obj.rotation[0], obj.rotation[1], obj.rotation[2]));
    glm::vec3 forward =
        glm::normalize(glm::vec3(obj.rotation[3], obj.rotation[4], obj.rotation[5]));
    glm::vec3 down =
        glm::normalize(glm::vec3(obj.rotation[6], obj.rotation[7], obj.rotation[8]));
  ...
  for (size_t p = 0; p < phoneData.numPhones; ++p) {
    ...
    std::cout << " " << uint16_t(phone.state) << " " << phone.position.x << " "
              << phone.position.y << " " << phone.position.z << std::endl;
  }
#endif
  ...
  for (int s = 0; s < restartData.numHospitals; ++s) {
    ...
    std::cout << " H " << p.position.x << " " << p.position.y << " " << p.position.z
              << std::endl;
  }
  for (int s = 0; s < restartData.numPolice; ++s) {
    Block9Restart& p = restartData.policeRestarts[s];
    std::cout << " P " << p.position.x << " " << p.position.y << " " << p.position.z
              << std::endl;
  }
#endif
  ...
  for (int i = 0; i < 23; ++i) {
    ...
    std::cout << pedTypeData.types[i].unknown1 << " " << pedTypeData.types[i].unknown7
              << std::endl;
  }
#endif
  ...
  state.scriptOnMissionFlag =
      (int32_t*)(state.script->getGlobals() + (size_t)scriptData.onMissionOffset);
  ...
  for (size_t g = 0; g < garageData.garageCount; ++g) {
    ...
    state.garages.push_back({g, glm::vec3(garage.x1, garage.y1, garage.z1),
                             glm::vec3(garage.x2, garage.y2, garage.z2), garage.type});
  }
  for (int c = 0; c < 18; ++c) {
    if (garageData.cars[c].modelId == 0) continue;
    auto& car = garageData.cars[c];
    glm::quat rotation(glm::mat3(glm::cross(car.rotation, glm::vec3(0.f, 0.f, 1.f)),
  ...
  for (unsigned g = 0; g < carGenerators.size(); ++g) {
    ...
    state.vehicleGenerators.emplace_back(
        g, gen.position, gen.angle, gen.modelId, gen.colourFG, gen.colourBG, gen.force,
