      ...
      if (entryDoor && entryDoor->constraint) {
        if (glm::length(d) <= 0.1f) {
          if (m_closeDoorTimer >= kCloseDoorIdleTime) {
            character->getCurrentVehicle()->setPartTarget(entryDoor, true,
                                                          entryDoor->closedAngle);
          }
          m_closeDoorTimer += dt;
        } else {
          m_closeDoorTimer = 0.f;
                                      ...
                                      CharacterController *controller)
{
  ...
  if (vehicle->vehicle->vehicletype == VehicleModelData::BOAT) {
  ...
  if (entering) {
    if (character->animator->getAnimation(AnimIndexAction) == anm_open) {
      if (character->animator->isCompleted(AnimIndexAction)) {
        tryToEnter = true;
                 ...
                 character->animator->getAnimationTime(AnimIndexAction) >= 0.5f) {
    ...
    if (targetDistance <= 0.4f) {
      ...
      character->setHeading(glm::degrees(atan2(targetDirection.y, targetDirection.x) -
                                         glm::half_pi<float>()));
