...
public:
  ...
  {
    ClosestNotMeRayResultCallback rayCallback(_vehicle->collision->getBulletBody(), from,
                                              to);
    const void* res = 0;
    ...
    , physVehicle(nullptr)
{
  ...
  if (collision->createPhysicsBody(this, data->modelName, nullptr, &info->handling)) {
    ...
    float travel =
        fabs(info->handling.suspensionUpperLimit - info->handling.suspensionLowerLimit);
    ...
    for (size_t w = 0; w < info->wheels.size(); ++w) {
      ...
      btWheelInfo& wi = physVehicle->addWheel(connection, btVector3(0.f, 0.f, -1.f),
                                              btVector3(1.f, 0.f, 0.f), restLength,
                                              data->wheelScale / 2.f, tuning, front);
      ...
      wi.m_frictionSlip = halfFriction +
...
void VehicleObject::tickPhysics(float dt)
{
  ...
  if (physVehicle) {
    ...
    for (int w = 0; w < physVehicle->getNumWheels(); ++w) {
      ...
      float brakeReal = 5.f * info->handling.brakeDeceleration *
      ...
      if (wi.m_bIsFrontWheel) {
        ...
        physVehicle->setSteeringValue(
            std::min(info->handling.steeringLock * (3.141f / 180.f),
                ...
                sign,
            w);
        // physVehicle->setSteeringValue(std::min(3.141f/2.f, std::abs(steerAngle)) *
        // sign, w);
      }
    }
    ...
    if (vehicle->vehicletype == VehicleModelData::BOAT) {
      if (isInWater()) {
        ...
        collision->getBulletBody()->applyForce(rudderVector * engineForce * 100.f,
                                               rudderPoint);
    ...
    auto wX =
        (int)((ws.x + WATER_WORLD_SIZE / 2.f) / (WATER_WORLD_SIZE / WATER_HQ_DATA_SIZE));
    auto wY =
        (int)((ws.y + WATER_WORLD_SIZE / 2.f) / (WATER_WORLD_SIZE / WATER_HQ_DATA_SIZE));
    ...
    for (auto& it : dynamicParts) {
      ...
      if (it.second.moveToAngle) {
        ...
        if (glm::abs(angledelta) <= 0.01f) {
          ...
          it.second.constraint->enableAngularMotor(true, glm::sign(angledelta) * 5.f,
                                                   1.f);
        }
      }
...
void VehicleObject::createObjectHinge(Part* part)
{
  ...
  auto hinge = new btHingeConstraint(*collision->getBulletBody(), *subObject,
                                     tr.getOrigin(), hingePosition, hingeAxis, hingeAxis);
