...
void ProjectileObject::checkPhysicsContact()
{
  ...
  for (int i = 0; i < numPairs; i++) {
    ...
    // unless we manually perform collision detection on this pair, the contacts are in
    // the dynamics world paircache:
...
void ProjectileObject::explode()
{
  if (!_exploded) {
    ...
    for (auto& o : engine->allObjects) {
      ...
      o->takeDamage({getPosition(), getPosition(), damage / glm::max(d, 1.f),
                     DamageInfo::Explosion, 0.f});
    }
    ...
    , _exploded(false)
{
  ...
  if (_info.type != Grenade) {
    ...
    engine->dynamicsWorld->addCollisionObject(
        _ghostBody, btBroadphaseProxy::SensorTrigger, btBroadphaseProxy::AllFilter);
  }
}
