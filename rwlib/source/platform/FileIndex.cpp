...
/**
 * Finds the 'real' case for a path, to get around the fact that Rockstar's data is
 * usually the wrong case.
...
std::string findPathRealCase(const std::string& base_src, const std::string& path_src)
{
  ...
  for (const path& path_component :
    ...
    // Search the current base path for a filename matching the component we're searching
    // for
    bool found = false;
